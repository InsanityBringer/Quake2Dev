...
void R_AddPoint(vec3_t pt, float s1, float t1, float s2, float t2);
void R_AddData(float f);
...
void R_AddData(float f)
{
	temp_verts[tvcurrent] = f;
	tvcurrent++;
	if (tvcurrent >= tvlast)
	{
		float *temp = (float*)realloc(temp_verts, (tvlast * 2) * sizeof(float));
		tvlast *= 2;
...
void R_AddPolygon(glpoly_t *poly)
{
	vec3_t pt;
	int i;

	//TODO: Polygons can be less than three verts?
	if (poly->numverts == 3) //a triangle, don't need to triangulate
	{
		for (i = 0; i < poly->numverts; i++)
		{
			...
			R_AddPoint(pt, poly->verts[i][3], poly->verts[i][4], poly->verts[i][5], poly->verts[i][6]);
		}
	}
	else //quad, must triangulate. triangulates as 0-1-2, 0-2-3
	{
		for (i = 0; i < 3; i++)
		{
			pt[0] = poly->verts[i][0];
		...
		for (i = 0; i < 3; i++)
		{
			int li = i;
			if (i != 0) li++;
			pt[0] = poly->verts[li][0];
			pt[1] = poly->verts[li][1];
			pt[2] = poly->verts[li][2];

			R_AddPoint(pt, poly->verts[li][3], poly->verts[li][4], poly->verts[li][5], poly->verts[li][6]);
		}
	}
}
...
void R_DrawLineBox(float minx, float miny, float minz, float maxx, float maxy, float maxz)
{
	qglBegin (GL_LINES);
	
	qglVertex3f (minx, miny, minz);
	qglVertex3f (maxx, miny, minz);

	qglVertex3f (minx, miny, minz);
	qglVertex3f (minx, maxy, minz);

	qglVertex3f (maxx, maxy, minz);
	qglVertex3f (minx, maxy, minz);

	qglVertex3f (maxx, maxy, minz);
	qglVertex3f (maxx, miny, minz);

	qglVertex3f (minx, miny, maxz);
	qglVertex3f (maxx, miny, maxz);

	qglVertex3f (minx, miny, maxz);
	qglVertex3f (minx, maxy, maxz);

	qglVertex3f (maxx, maxy, maxz);
	qglVertex3f (minx, maxy, maxz);
...
void R_DrawLeafOutlines (void)
{
	...
	//view the current leaf from which you are viewing
	if (gl_leafvis->value == 1)
	{
		minx = current_viewleaf->minmaxs[0];
		miny = current_viewleaf->minmaxs[1];
		minz = current_viewleaf->minmaxs[2];
		maxx = current_viewleaf->minmaxs[3];
		maxy = current_viewleaf->minmaxs[4];
		maxz = current_viewleaf->minmaxs[5];

		R_DrawLineBox(minx, miny, minz, maxx, maxy, maxz);
	}
	//view every node ever
	else if (gl_leafvis->value == 3)
	{
	...
	//only view visible leaves
	else if (gl_leafvis->value == 2)
	{
		for (i=0,leaf=r_worldmodel->leafs ; i<r_worldmodel->numleafs ; i++, leaf++)
...
dynamic:
	...
	//special lightmapping routine for dynamically lit surfaces
	if (is_dynamic)
	{
...
//==========
	...
	else //surface not hit by a dynamic light
	{
		c_brush_polys++;
...
//==========
			...
			{
				v = p->verts[0];
				switch ((int)gl_polygonmode->value)
				{
				case 0:
					qglBegin (GL_POLYGON);
					break;
...
void R_DrawWorldDebugVBO(void)
{
	...
	for (i = 0, surface = r_worldmodel->surfaces; i < r_worldmodel->numsurfaces; i++, surface++)
	{
...
void R_DrawWorld (void)
{
	...
	//If multitexturing is enabled, render the world in one run
	if (qglMTexCoord2fSGIS)
	{
		GL_EnableMultitexture( true );

		//Ensure that the texturing mode for the normal textures is correct
		GL_SelectTexture(GL_TEXTURE0);
		GL_TexEnv(GL_REPLACE);

		GL_SelectTexture(GL_TEXTURE1);

		//Debug feature to render only lightmaps
		if ( gl_lightmap->value )
			GL_TexEnv(GL_REPLACE);
		//Otherwise, blend the lightmaps
		else 
			GL_TexEnv(GL_MODULATE);

		//Traverse the nodes and render all normal surfaces
	...
	{
		//With multitexturing off, this just processes data
		R_RecursiveWorldNode (r_worldmodel->nodes);
	}
	...
	*/
	//draw all the normal textures
	DrawTextureChains();
	//draw all the lightmaps as a seperate pass
	R_BlendLightmaps();
