...
//[IB]
cvar_t	*gl_polygonmode;
cvar_t	*gl_debugvbo;
cvar_t* gl_fps;

//[ISB] horrid hack to estimate framerate
...
void R_Clear (void)
{
	qglClearColor(0.0f, 0.0f, 0.05f, 1.0f);
	if (gl_ztrick->value)
	{
...
void R_RenderView (refdef_t *fd)
{
	...
	R_SetFrustum ();
	//set projection and modelview matricies
	R_SetupGL ();
	//decompress PVS, mark all visible leaves
	R_MarkLeaves ();	// done here so we know if we're in water
	//performs actual world drawing
	R_DrawWorld ();

	//draw visible entities
	R_DrawEntitiesOnList ();
	//perform the actual blending of dynamic lights
	R_RenderDlights ();
...
void R_Register( void )
{
	...
	//[ib]
	gl_leafvis = ri.Cvar_Get ("gl_leafvis", "0", 0);
	...
	gl_polygonmode = ri.Cvar_Get("gl_polygonmode", "0", 0);
	gl_debugvbo = ri.Cvar_Get("gl_debugvbo", "0", 0);
	gl_fps = ri.Cvar_Get("gl_fps", "0", 0);
...
{	
	...
	*/
	...
	//ri.Con_Printf(PRINT_ALL, "GL_EXTENSIONS: There's too many to list. heh\n");
	...
	*/
	...
	if (strstr(gl_config.extensions_string, "ARB_texture_non_power_of_two"))
	{
		ri.Con_Printf(PRINT_ALL, "...enabling ARB_texture_non_power_of_two\n");
		useNPO2 = true;
	}
	else
	{
		ri.Con_Printf(PRINT_ALL, "...ARB_texture_non_power_of_two not found\n");
	}
...
#define NUM_BEAM_SEGS 6
	...
	qglEnable( GL_BLEND );
	qglDepthMask( GL_FALSE );
	//qglBlendFunc(GL_SRC_ALPHA, GL_ONE); //[ISB] additive rendering experiment
