...
extern LARGE_INTEGER oldtime, timer_rate;
extern cvar_t* gl_fps;
char fpsbuf[64];
...
void GLimp_EndFrame (void)
{
	...
	//[ISB] horrible FPS estimate
	if (gl_fps->value)
	{
		LARGE_INTEGER newtime;
		QueryPerformanceCounter(&newtime);
		LONGLONG delta = newtime.QuadPart - oldtime.QuadPart;
		oldtime = newtime;

		double time = delta / (double)timer_rate.QuadPart;
		double fps = 1.0 / time;

		snprintf(fpsbuf, sizeof(fpsbuf) - 1, "fps: %.2f", fps);
		fpsbuf[63] = '\0';

		int startx = vid.width - 160;
		int len = strlen(fpsbuf);
		for (int i = 0; i < len; i++)
		{
			Draw_Char(startx, 0, fpsbuf[i]);
			startx += 8;
		}
	}
